...
#define BLE_UUID_COM_SERVICE 0xFF20                      /**< The UUID of the Nordic UART Service. */
...
typedef struct ble_com_s ble_com_t;

/**@brief Nordic UART Service event handler type. */
typedef void (*ble_com_data_handler_t) (ble_com_t * p_com, uint8_t * p_data, uint16_t length);

/**@brief Nordic UART Service initialization structure.
 *
 * @details This structure contains the initialization information for the service. The application
 * must fill this structure and pass it to the service using the @ref ble_com_init
 *          function.
 */
typedef struct
{
 ...
 *
 * @param[out] p_com      Nordic UART Service structure. This structure must be supplied
 *                        by the application. It is initialized by this function and will
 *                        later be used to identify this particular service instance.
 * @param[in] p_com_init  Information needed to initialize the service.
 *
 * @retval NRF_SUCCESS If the service was successfully initialized. Otherwise, an error code is returned.
 * @retval NRF_ERROR_NULL If either of the pointers p_com or p_com_init is NULL.
 */
 ...
 *          peer.
 *
 * @param[in] p_com       Pointer to the Nordic UART Service structure.
